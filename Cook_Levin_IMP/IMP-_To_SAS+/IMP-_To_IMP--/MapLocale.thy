...
definition "map'_f_IMP_loop_body \<equiv>
  ...
  invoke_subprogram f_pref_str f_IMP_Minus;;
...
definition map'_f_IMP_Minus where
  "map'_f_IMP_Minus \<equiv>
...
lemma neq_prefix_imp_test[simp, intro]: "\<not> prefix x z \<Longrightarrow> add_prefix x y \<noteq> z"
  by auto
lemma map'_f_IMP_Minus_correct_function:
  "(invoke_subprogram p map'_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     s' (add_prefix p map'_f_acc_str)
  ...
  apply(simp only: map'_f_IMP_Minus_def prefix_simps)
  ...
  subgoal for s s' t x s2 y xa s2a ya x' s2b yb xb s2c yc
    ...
    apply(simp only: prefix_simps)
    apply(vcg map'_f_IMP_vars functional_correctness: f_IMP_Minus_correct)
...
lemma map'_f_IMP_Minus_correct_effects:
  "\<lbrakk>(invoke_subprogram (p @ map'_f_pref) map'_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    v \<in> vars; \<not> (prefix map'_f_pref v)\<rbrakk>
  \<Longrightarrow> s (add_prefix p v) = s' (add_prefix p v)"
  using com_add_prefix_valid'' com_only_vars prefix_def
  by blast

lemma map'_f_IMP_Minus_correct_time_loop_condition:
...
lemma map'_f_IMP_Minus_correct_time:
  "(invoke_subprogram p map'_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
  ...
  proof-
    ...
    proof(cases "0 < map'_f_compute_loop_condition (map'_f_imp_to_HOL_state p s) ")
      ...
      show ?thesis
        ...
        apply (simp only: prefix_simps)
        apply(vcg_time map'_f_IMP_vars functional_correctness: f_IMP_Minus_correct)
...
lemma map'_f_IMP_Minus_correct:
  "\<lbrakk>(invoke_subprogram (p1 @ p2) map'_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    \<And>v. v \<in> vars \<Longrightarrow> \<not> (prefix p2 v);
     \<lbrakk>t = (map'_f_imp_time 0 (map'_f_imp_to_HOL_state (p1 @ p2) s));
      ...
      \<And>v. v \<in> vars \<Longrightarrow> s (add_prefix p1 v) = s' (add_prefix p1 v)\<rbrakk>
     \<Longrightarrow> P\<rbrakk> \<Longrightarrow> P"
  using map'_f_IMP_Minus_correct_function
  by (auto simp: map'_f_IMP_Minus_correct_time)
    (meson map'_f_IMP_Minus_correct_effects set_mono_prefix)
...
(* 
      ...
      reverse_nat_state = \<lparr>reverse_nat_n = reverse_nat_n, reverse_nat_ret = reverse_nat_ret'\<rparr>;
      reverse_nat_ret'' = reverse_nat_imp reverse_nat_state;*)
definition map_f_IMP_Minus where
  "map_f_IMP_Minus \<equiv>

  (map'_f_pref_str @ map'_f_xs_str) ::= (A (V map_f_xs_str));;
  (map'_f_pref_str @ map'_f_acc_str) ::= (A (N 0));;
  invoke_subprogram map'_f_pref_str map'_f_IMP_Minus;;
...
lemma map_f_IMP_Minus_correct_function:
  "(invoke_subprogram p map_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     s' (add_prefix p map_f_ret_str)
      = map_f_state_ret (map_f_imp (map_f_imp_to_HOL_state p s))"
  by (fastforce elim: map'_f_IMP_Minus_correct reverse_nat_IMP_Minus_correct simp: map_f_imp_to_HOL_state_def
      map_f_imp.simps map_f_state_upd_def map'_f_imp_to_HOL_state_def
      map_f_IMP_Minus_def 
      reverse_nat_imp_to_HOL_state_def
      invoke_subprogram_append)

lemma map_f_IMP_Minus_correct_effects:
  "\<lbrakk>(invoke_subprogram (p @ map_f_pref) map_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    v \<in> vars; \<not> (prefix map_f_pref v)\<rbrakk>
  \<Longrightarrow> s (add_prefix p v) = s' (add_prefix p v)"
  using com_add_prefix_valid'' com_only_vars prefix_def
  by blast

lemma map_f_IMP_Minus_correct_time:
  "(invoke_subprogram p map_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     t = map_f_imp_time 0 (map_f_imp_to_HOL_state p s)"
  by (fastforce elim: map'_f_IMP_Minus_correct reverse_nat_IMP_Minus_correct simp: map_f_imp_to_HOL_state_def
      map_f_imp_time.simps map'_f_imp_to_HOL_state_def map_f_IMP_Minus_def reverse_nat_imp_to_HOL_state_def
      invoke_subprogram_append Let_def)

lemma map_f_IMP_Minus_correct[functional_correctness]:
  "\<lbrakk>(invoke_subprogram (p1 @ p2) map_f_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    \<And>v. v \<in> vars \<Longrightarrow> \<not> (prefix p2 v);
     \<lbrakk>t = (map_f_imp_time 0 (map_f_imp_to_HOL_state (p1 @ p2) s));
      ...
      \<And>v. v \<in> vars \<Longrightarrow> s (add_prefix p1 v) = s' (add_prefix p1 v)\<rbrakk>
     \<Longrightarrow> P\<rbrakk> \<Longrightarrow> P"
  using map_f_IMP_Minus_correct_time map_f_IMP_Minus_correct_function
    map_f_IMP_Minus_correct_effects
...
definition one_to_two_IMP_Minus where
  "one_to_two_IMP_Minus \<equiv>
...
lemma one_to_two_IMP_Minus_correct_function:
  "(invoke_subprogram p one_to_two_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     s' (add_prefix p one_to_two_out_str)
      = one_to_two_state_out
 (one_to_two_imp (one_to_two_imp_to_HOL_state p s))"
  apply (subst (asm) one_to_two_IMP_Minus_def)
  apply(simp only: prefix_simps)
  apply (erule Seq_tE)+
  apply (erule f_IMP_Minus_correct[where vars=one_to_two_IMP_vars])
   apply simp
  thm functional_correctness
  apply (remove_assign)
  apply (unfold_stuff state_translators: one_to_two_imp_to_HOL_state_def f_imp_to_HOL_state_def)
  by (auto elim: f_IMP_Minus_correct simp add: one_to_two_imp_to_HOL_state_def
      one_to_two_imp.simps one_to_two_state_upd_def
      one_to_two_IMP_Minus_def invoke_subprogram_append)
  
lemma one_to_two_IMP_Minus_correct_effects:
  "\<lbrakk>(invoke_subprogram (p @ one_to_two_pref) one_to_two_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    v \<in> vars; \<not> (prefix one_to_two_pref v)\<rbrakk>
  \<Longrightarrow> s (add_prefix p v) = s' (add_prefix p v)"
  using com_add_prefix_valid'' com_only_vars prefix_def
  by blast

lemma one_to_two_IMP_Minus_correct_time:
  "(invoke_subprogram p one_to_two_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s' \<Longrightarrow>
     t = one_to_two_imp_time 0 (one_to_two_imp_to_HOL_state p s)"
  by (fastforce elim: f_IMP_Minus_correct simp: one_to_two_imp_to_HOL_state_def
      one_to_two_imp_time.simps f_imp_to_HOL_state_def one_to_two_IMP_Minus_def
      invoke_subprogram_append)

lemma one_to_two_IMP_Minus_correct[functional_correctness]:
  "\<lbrakk>(invoke_subprogram (p1 @ p2) one_to_two_IMP_Minus, s) \<Rightarrow>\<^bsup>t\<^esup> s';
    \<And>v. v \<in> vars \<Longrightarrow> \<not> (prefix p2 v);
     \<lbrakk>t = (one_to_two_imp_time 0 (one_to_two_imp_to_HOL_state (p1 @ p2) s));
      ...
      \<And>v. v \<in> vars \<Longrightarrow> s (add_prefix p1 v) = s' (add_prefix p1 v)\<rbrakk>
     \<Longrightarrow> P\<rbrakk> \<Longrightarrow> P"
  using one_to_two_IMP_Minus_correct_time one_to_two_IMP_Minus_correct_function
    one_to_two_IMP_Minus_correct_effects 
  by (meson set_mono_prefix)
...
interpretation snd'_nat_two_var_version: 
  ...
  snd'_p_str "CHR ''$''" snd'_IMP_Minus snd'_imp_to_HOL_state
  apply standard
  ...
  using  snd'_IMP_Minus_correct apply (auto simp add: snd'_state.defs) by blast

thm snd'_nat_two_var_version.one_to_two_IMP_Minus_correct
...
interpretation 
  ...
  "CHR ''&''" snd'_nat_two_var_version.one_to_two_IMP_Minus snd'_nat_two_var_version.one_to_two_imp_to_HOL_state
  apply standard
  ...
  subgoal for p1 p2 S t s' vars P
    using [[unify_trace_failure]] apply (rule 
        one_var_to_two_var.one_to_two_IMP_Minus_correct[OF snd'_nat_two_var_version.one_var_to_two_var_axioms,
          of p1 p2 S t s' vars P]) by blast+ 
  done

(* map snd is refined! *)

term map_snd.map_f_IMP_Minus
thm map_snd.map_f_IMP_Minus_def
thm map_snd.map_f_IMP_Minus_correct
