...
" write_nat_tape 0 k = [write_end_tape k]"|
" write_nat_tape n k =  (
...
*)
(*
fun symbol_to_bit::" nat\<Rightarrow>nat" where
"symbol_to_bit \<box> =0"|
"symbol_to_bit \<zero> =0"|
"symbol_to_bit \<one> =1"|
"symbol_to_bit _=0"
*)
...
(*
fun add_bit_tape::"nat \<Rightarrow>nat\<Rightarrow>nat \<Rightarrow>command" where
"write_bit_tape b k=( \<lambda> symbols_read_from_tapes.
      (
        let a1= symbols_read_from_tapes! k1 ;
          a2= symbols_read_from_tapes! k2
        in
        if a1=0 \<and> a2=0 then 2
       else 0
...
*)
abbreviation ADD_COM1::"nat\<Rightarrow>command" where
"ADD_COM1 k3\<equiv>( \<lambda> symbols_read_from_tapes.
       let a1= symbols_read_from_tapes! 1 ;
          a2= symbols_read_from_tapes! 2
        in
      (
        if a1=0\<and>a2=0 then 2
        else if (a1-2)+(a2-2)\<ge>2 then 1
        else 0
        , 
        let len =(length symbols_read_from_tapes);
         new_symbols_of_tapes =(update_list k3 ((a1+a2) mod 2+2) symbols_read_from_tapes);
        directions =[Stay,Right,Right]@(replicate (k3-3) Stay)@ [Right]@(replicate  (len-k3) Stay)
          in
       (zip new_symbols_of_tapes  directions)
      )
   )
 "
abbreviation ADD_COM2::"nat\<Rightarrow>command" where
"ADD_COM2 k3\<equiv>( \<lambda> symbols_read_from_tapes.
       let a1= symbols_read_from_tapes! 1 ;
          a2= symbols_read_from_tapes! 2
        in
      (
        if a1=0\<and>a2=0 then 2
        else if ((a1-2)+(a2-2)+1\<ge>2) then 1
        else 0
        , 
        let len =(length symbols_read_from_tapes);
         new_symbols_of_tapes =(update_list k3 
        (if a1=0\<and>a2=0 then 0
        else 
         ((a1+a2+1) mod 2+2) )
        symbols_read_from_tapes);
        directions =[Stay,Right,Right]@(replicate (k3-3) Stay)@ [Right]@(replicate  (len-k3) Stay)
          in
       (zip new_symbols_of_tapes  directions)
      )
...
abbreviation ADD_TM::"nat\<Rightarrow>command list" where
"ADD_TM k3\<equiv> [ADD_COM1 k3,ADD_COM2 k3]"
...
(*
fun read_nat_tape::"tape list \<Rightarrow>nat \<Rightarrow>nat" where
"read_nat_tape
*)
(*
...
lemma write_end_tape_wf_command_x:
  fixes k K Q
  assumes "k \<ge> 1" and "K \<ge> k" and "Q \<ge> 4"
  shows "wf_command K Q (write_end_tape k)"
  using assms
...
definition clear_command::"nat \<Rightarrow>command" where
"clear_command kth = ( \<lambda> symbols_read_from_tapes.
     let len =(length symbols_read_from_tapes)
      in
        (
        if symbols_read_from_tapes!kth = 0 then 
        (1,
            let directions = (replicate  (kth-1) Stay)@ [Right]@(replicate  (len-kth) Stay)
            in
            (zip symbols_read_from_tapes  directions)
        )
      else 
        (0,
            let directions = (replicate  (kth-1) Stay)@ [Left]@(replicate  (len-kth) Stay)
            in
            (zip symbols_read_from_tapes  directions)
        )
    )
...
definition clear_machine::"nat\<Rightarrow> machine" where
"clear_machine k= [clear_command k]"
...
lemma l2:"length gs  \<ge> kth\<and>kth \<ge> 1\<longrightarrow>length gs = length ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay))"
  apply simp  
  by (simp add: less_eq_Suc_le)

lemma wf_clear_command:
  fixes K kth 
  assumes "K \<ge> kth" and "kth \<ge> 1"
  shows "wf_command K 1 (clear_command kth)"
proof (subst wf_command_def, rule allI, rule impI)
  fix gs :: "nat list"
  assume "length gs = K"
  show "length (snd (clear_command kth gs)) = K \<and> fst (clear_command kth gs) \<le> 1"
  proof (cases "gs ! kth = 0")
    case True
    then have "clear_command kth gs = (1, let directions = (replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay) in zip gs directions)"
      by (simp add: clear_command_def)
    then have fst_part: "fst (clear_command kth gs) = 1"
      by simp
    have "length (snd (clear_command kth gs)) = length (zip gs ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay)))"
      by (simp add: True clear_command_def)
    moreover have "length gs = length ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay))"
      using \<open>length gs = K\<close> l2
    using assms(1) assms(2) by blast
    ultimately have snd_part: "length (snd (clear_command kth gs)) = K" by (simp add: \<open>length gs = K\<close>)
    from fst_part snd_part show ?thesis
      by simp
  next
    case False
    then have "clear_command kth gs = (0, let directions = (replicate (kth - 1) Stay) @ [Left] @ (replicate (length gs - kth) Stay) in zip gs directions)"
      by (simp add: clear_command_def)
    then have fst_part: "fst (clear_command kth gs) = 0"
      by simp
    have "length (snd (clear_command kth gs)) = length (zip gs ((replicate (kth - 1) Stay) @ [Left] @ (replicate (length gs - kth) Stay)))"
      by (simp add: False clear_command_def)
    moreover have "length gs = length ((replicate (kth - 1) Stay) @ [Left] @ (replicate (length gs - kth) Stay))"
      using assms(1) \<open>length gs = K\<close> using assms(2) by force
    ultimately have snd_part: "length (snd (clear_command kth gs)) = K"
      by (simp add: \<open>length gs = K\<close>)
    from fst_part snd_part show ?thesis
      by simp
  qed
...
(*
lemma turing_clear_command:
  fixes K kth G
  assumes "K \<ge> kth" and "kth \<ge> 1" and "G \<ge> 4"
  shows "turing_command K 1 G (clear_command kth)"
proof (subst turing_command_def, rule conjI)
  show "wf_command K 1 (clear_command kth)"
  proof (subst wf_command_def, rule allI, rule impI)
    fix gs :: "nat list"
    assume "length gs = K"
    show "length (snd (clear_command kth gs)) = K \<and> fst (clear_command kth gs) \<le> 1"
    proof (cases "gs ! kth = 0")
      ...
      have "length (snd (clear_command kth gs)) = length (zip gs ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay)))"
        by (simp add: True clear_command_def)
      moreover have "length gs = length ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay))"
        using assms(1) \<open>length gs = K\<close> by simp
      ultimately have snd_part: "length (snd (clear_command kth gs)) = K"
        by (simp add: \<open>length gs = K\<close>)
      from fst_part snd_part show ?thesis
        by simp
  ...
  qed
next
  show "\<forall>gs. length gs = K \<longrightarrow>
         ((\<forall>i<K. gs ! i < G) \<longrightarrow> (\<forall>i<K. fst (snd (clear_command kth gs) ! i) < G)) \<and>
         (0 < K \<longrightarrow> fst (snd (clear_command kth gs) ! 0) = gs ! 0)"
  ...
  proof (subst wf_command_def, rule allI, rule impI)
    ...
    proof (cases "gs ! kth = 0")
      ...
      have "length (snd (clear_command kth gs)) = length (zip gs ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay)))"
        by (simp add: True clear_command_def)
      moreover have "length gs = length ((replicate (kth - 1) Stay) @ [Right] @ (replicate (length gs - kth) Stay))"
        using assms(1) \<open>length gs = K\<close>  assms(2) l2 by blast
      ultimately have snd_part: "length (snd (clear_command kth gs)) = K"
        by (simp add: \<open>length gs = K\<close>)
      from fst_part snd_part show ?thesis
        by simp
    next
      ...
      have "length (snd (clear_command kth gs)) = length (zip gs ((replicate (kth - 1) Stay) @ [Left] @ (replicate (length gs - kth) Stay)))"
        by (simp add: False clear_command_def)
      moreover have "length gs = length ((replicate (kth - 1) Stay) @ [Left] @ (replicate (length gs - kth) Stay))"
        using assms(1) \<open>length gs = K\<close> assms(2) by auto
      ultimately have snd_part: "length (snd (clear_command kth gs)) = K"
        by (simp add: \<open>length gs = K\<close>)
      from fst_part snd_part show ?thesis
        by simp
...
next
  show " \<forall>gs. length gs = K \<longrightarrow>
         ((\<forall>i<K. gs ! i < G) \<longrightarrow> (\<forall>i<K. clear_command kth gs [.] i < G)) \<and>
         (0 < K \<longrightarrow> clear_command kth gs [.] 0 = gs ! 0)"
  proof (rule allI, rule impI)
    fix gs :: "nat list"
    assume "length gs = K" 
    show "((\<forall>i<K. gs ! i < G) \<longrightarrow> (\<forall>i<K. clear_command kth gs [.] i < G)) \<and>
          (0 < K \<longrightarrow> clear_command kth gs [.] 0 = gs ! 0)"
...
(*TODO*)
fun TM_to_state::"tape list\<Rightarrow>nat\<Rightarrow>(nat \<Rightarrow>nat) \<Rightarrow>(vname\<Rightarrow>nat)\<Rightarrow>bool" where
"TM_to_state tps n rind s=True"

abbreviation TP0::"tape" where
"TP0\<equiv> ((\<lambda>x.0),1)"

abbreviation TPS0::"nat \<Rightarrow>tape list" where
"TPS0 n\<equiv>replicate n TP0 "

lemma TPS0_to_state :"TM_to_state (TPS0 n) (\<lambda>x. 0)"
  sorry

theorem WHILE_to_TM_correct_and_in_polynomial_time:
  fixes t::"vname\<Rightarrow>nat"
  and n::"nat"
  and c::"com"
assumes " (c,(\<lambda>x.0)) \<Rightarrow>\<^bsup> n \<^esup> t"

  obtains tps'
  where "\<exists>T. transforms M TPS0 T tps'"
  and "TM_to_state tps' t"
