...
definition "vc_to_fas \<equiv> \<lambda>(E,K). (if finite E \<and> K \<le> card (\<Union>E) \<and> (\<forall>e \<in> E. card e = 2)
...
proof (cases "finite E \<and> k \<le> card (\<Union>E) \<and> (\<forall>e \<in> E. card e = 2)")
  case True
