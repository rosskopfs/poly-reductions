theory XC_To_ST
  imports "../X3C_To_ST/ST_Definition"
...
definition X3C_to_steiner_tree ::
   ...
   ( (a ` X) \<union> {ROOT} \<union> (c ` S),
     ...
     {ROOT} \<union>  (a ` X),
   if C > 0 then (C + 1) * card X div C else 1))"
...
proof -
  ...
  let ?C = "Max (card ` S)"
  let ?k = "if ?C > 0 then (?C + 1) * card X div ?C else 1"
  ...
  proof -
    have "\<forall>C\<in>S'. card C \<le> ?C" using \<open>S' \<subseteq> S\<close>  
      by (simp add: \<open>finite S\<close> in_mono)
    then have "sum card S' \<le> ?C * card S'" 
      by (metis id_apply mult.commute of_nat_eq_id sum_bounded_above)
    moreover have "(\<Sum>e \<in> Te. ?w  e) = card Te"  by fastforce
    ultimately show ?thesis (* ?A \<le> ?C  *  *)
      using card_Te sum_card_S' 
      (* 
         \<Longrightarrow> card X/C \<le> card S'
         \<Longrightarrow> card Te \<le> card X +  (card S' div C) 
      *)
  qed
