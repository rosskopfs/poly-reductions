...
theorem is_reduction_sat_is: "is_reduction sat_is three_cnf_sat independent_set"
proof (rule is_reductionI)
  ...
  with \<open>sat_is F = _\<close> have "length F = k" unfolding sat_is_def using \<open>F \<in> three_cnf_sat\<close> by auto
...
next
  ...
  proof (cases "is_n_sat 3 F")
    ...
    with wf show "F \<in> three_cnf_sat" by auto
  next
    case False
    with \<open>sat_is F \<in> _\<close> show "F \<in> three_cnf_sat" unfolding sat_is_def by auto
  qed
qed
