...
definition "mop_sat_to_at_most_three_sat_aux xs i =
  REST [ sat_to_at_most_three_sat_aux xs i \<mapsto> (sum_list (map (\<lambda>xs. length xs^3) xs) + length xs)^2 ]"

definition "sat_to_at_most_three_sat_poly \<equiv> \<lambda> F. do {
  res \<leftarrow> mop_sat_to_at_most_three_sat_aux (V F) 0;
  RETURNT res
}"

definition "size_SAT xs \<equiv> sum_list (map length xs) + length xs"
definition "size_AT_MOST_THREE_SAT xs \<equiv> sum_list (map length xs) + length xs"

(* TODO: space *)
definition "sat_to_at_most_three_sat_space n \<equiv> n^3"
definition "sat_to_at_most_three_sat_time n \<equiv> n^6"

lemma length_bound_to_at_most_3_clause: "length (to_at_most_3_clause xs i j) \<le> (1 + length xs^2)"
proof (induction xs i j rule: to_at_most_3_clause.induct)
  ...
  from 1 have IH: "?rec \<le> 1 + (3 + length rest)^2" by (simp add: numeral_Bit1)
  have "length (to_at_most_3_clause (a # b # c # d # rest) i j) = 3 + ?rec + length rest" by simp
  also have "... \<le> 4 + (3 + length rest)^2 + length rest" using IH by simp
  also have "... = 13 + 7 * length rest + length rest^2" by algebra
  also have "... \<le> 17 + 8 * length rest + length rest^2" by linarith
...
lemma sat_to_at_most_three_sat_size:
  "size_AT_MOST_THREE_SAT (sat_to_at_most_three_sat xs) \<le> sat_to_at_most_three_sat_size (size_SAT xs)"
sorry

lemma sum_pow3_leq: "(x :: nat)^3 + y^3 \<le> (x + y)^3"
proof -
...
lemma sum_list_pow_3: "sum_list (map (\<lambda>xs. (f::'a \<Rightarrow> nat) xs^3) xs) \<le> (sum_list (map f xs))^3"
proof (induction xs)
  case (Cons a xs)
  then have "(\<Sum>xs\<leftarrow>a # xs. f xs ^ 3) = f a ^ 3 + (\<Sum>xs\<leftarrow>xs. f xs ^ 3)" by force
  also have "... \<le> f a ^ 3 + (sum_list (map f xs))^3" using Cons by force
  also have "... \<le> (f a + sum_list (map f xs))^3" using sum_pow3_leq by fast
...
lemma sat_to_at_most_three_sat_refines:
  ...
  apply simp
  subgoal proof -
    have "(sum_list (map (((\<lambda>xs. length xs ^ 3) \<circ>\<circ> map) (map_lit RV)) F))^2 =
        sum_list (map (\<lambda>xs. length xs ^ 3) F)^2"
      by (simp add: comp_def)
    also have "... \<le> ((sum_list (map length F))^3)^2"
      using sum_list_pow_3[of length F] power2_nat_le_eq_le[symmetric] by blast
    also have "... = (sum_list (map length F))^6" by force
    finally have "(sum_list (map (((\<lambda>xs. length xs ^ 3) \<circ>\<circ> map) (map_lit RV)) F))^2 \<le> 
