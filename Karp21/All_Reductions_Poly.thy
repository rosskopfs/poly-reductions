theory All_Reductions_Poly
  imports
    ...
    SAT_To_AT_MOST_THREE_SAT_Poly
    AT_MOST_THREE_SAT_To_THREE_SAT_Poly
...
text \<open>
  \<^item> SAT
    \<^item> AM3-SAT @{thm sat_to_at_most_three_sat_ispolyred} (* TODO: missing space bound *)
      \<^item> 3-SAT @{thm at_most_three_sat_to_three_sat_ispolyred}
    ...
    \<^item> Exact Cover @{thm sat_to_xc_ispolyred}
      ...
      \<^item> Three-Dimensional Matching @{thm xc_to_three_dm_ispolyred} (* TODO: missing space bound *)
        \<^item> Exact Cover by 3-Sets @{thm tdm_to_x3c_is_polyred}
          \<^item> Steiner Tree @{thm X3C_to_steiner_tree_ispolyred} (* TODO: missing space bound *)
  ...
  \<^item> Independent Set @{thm sat_to_is_ispolyred}
    \<^item> Vertex Cover @{thm is_to_vc_ispolyred}
      ...
      \<^item> Feedback Arc Set @{thm vc_to_fas_ispolyred}
