...
lemma rev_eq_rev_aux0: "List.rev xs = rev_aux [] xs"
...
(* Currently we have a problem with the 'unconditional_nat', which is supposed to
   remove the assumptions and TYPE arguments generated by 'function_compile_nat'.
   If we use it to transform f into f', and want to use it to transform g, which calls f,
   into g', then g' will must somehow reference f' instead of f.
   Thus, we can't use the command on functions that use generated helper functions. *)
...
(* TODO: ( <= ) needs helper function leq_aux *)
